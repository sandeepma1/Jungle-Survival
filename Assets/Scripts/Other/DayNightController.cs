...
public class DayNightController : MonoBehaviour
{
		...
		GameObject dayNightImage;
		/// <summary>
		/// Initializes working variables and performs starting calculations.
		/// </summary>
		void Initialize ()
		{
				timer = dayCycleLength;
				dayNightImage = GameObject.Find ("DNImage");
				...
				currentDay = GameObject.Find ("CurrentDay");
				//currentDay.GetComponent<TextMesh> ().text = day.ToString ();
		...
		void Update ()
		{
				...
				dayNightImage.transform.localRotation = Quaternion.Euler (0f, 0f, -currentCycleTime);
		}
		...
		void LateUpdate ()
		{
				if (GameEventManager.GetState () == GameEventManager.E_STATES.e_game || GameEventManager.GetState () == GameEventManager.E_STATES.e_actionButtonPressed) {
						dayNightImage.GetComponent<Renderer>().enabled = true;
				} else
						dayNightImage.GetComponent<Renderer>().enabled = false;
		}
